...
from utils.triplet_loss_v3 import corrective_triplet_loss_v3
...
class Trainer(TrainBase):
    ...
    def _init_model(self):
        ...
        self.optimizer = BertAdam([
                    ...
                    {'params': self.model.image_linear.parameters(), 'lr': self.args.lr},
                    {'params': self.model.text_linear.parameters(), 'lr': self.args.lr}
        ...
        # self.hessian_calculator = hessian_calculators[self.args.loss](
        #     wrt="weight",
        #     shape="diagonal",
        #     speed="half",
        #     method=self.args.loss_approx,
        # )
    ...
    def _init_dataset(self):
        ...
        if self.args.dataset == 'flickr':
            data_dir = '/home/yuebai/Data/Dataset/CrossModal/MIRFLICKR-25K'
        elif self.args.dataset == 'coco':
            data_dir = '/home/yuebai/Data/Dataset/CrossModal/MS-COCO'
        elif self.args.dataset == 'nuswide':
            data_dir = '/home/yuebai/Data/Dataset/CrossModal/NUS-WIDE'
        else:
            raise ValueError("Unknown dataset")
        self.logger.info(f"Using {self.args.dataset} dataset.")
        self.args.index_file = os.path.join(data_dir, self.args.index_file)
        self.args.caption_file = os.path.join(data_dir, self.args.caption_file)
        self.args.label_file = os.path.join(data_dir, self.args.label_file)
        train_data, query_data, retrieval_data = dataloader(captionFile=self.args.caption_file, 
                                        ...
                                        seed=self.args.seed)
        self.train_labels = train_data.get_all_label().cuda()
    ...
    def train_epoch(self, epoch):
        ...
        for image, text, label, index in self.train_loader:
            ...
            # shape of hash_img & hash_test is (batch_size * batch_size)

            # loss = self.hyp(hash_img, hash_text, label)


            embeddings = torch.nn.functional.normalize(hash_img, dim=1, eps=1e-10)
            embeddings_t = torch.nn.functional.normalize(hash_text, dim=1, eps=1e-10)
            loss = corrective_triplet_loss_v3(embeddings, embeddings_t, label)

            # V = np.zeros((opt.num_database, opt.hash_bit))
            # loss_asy = losses[1](fea_hash, V[ind.cpu().numpy(), :], label, opt.nclass)
            # loss_cla = smooth_CE(fea_cls, label, peak=0.9)

            # loss = loss_asy + 20 * loss_cla + loss_cortriplet
